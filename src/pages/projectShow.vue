<template>
  ...
  <ChildOne @button-click="(n)=>{console.log(n)}" :size="300" class="button_1" :data="[{number:1,date:'2022-01-01'},{number:2,date:'2022-01-02'}]"><template #default="data">{{ data.number }}{{ data.date }}</template>
  <template #for="i">{{ i.number }}--{{ i.date }}<hr/></template></ChildOne>
  <Docx></Docx>
